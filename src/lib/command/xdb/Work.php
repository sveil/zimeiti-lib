...
class Work extends Command
{
    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        if (empty($this->id)) {
            ...
            try {
                $queue = Db::name($this->table)->where(['id' => $this->id, 'status' => '1'])->find();
                ...
                if (empty($queue)) {
                    ...
                    // Lock task status
                    Db::name($this->table)->where(['id' => $queue['id']])->update(['status' => '2', 'start_at' => date('Y-m-d H:i:s')]);
    ...
    protected function update($status, $message)
    {
        ...
        $result = Db::name($this->table)->where(['id' => $this->id])->update([
